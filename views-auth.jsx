...
const AuthPage = ({ initialMode = 'signin', onSignedIn, onSignedUp }) => {
  ...
  const checkHandle = (value) => {
    ...
    handleTimer.current = setTimeout(async () => {
      const result = await supabaseService?.checkHandle(value);
  ...
  const valid = isSignup
    ...
    : email.trim().length >= 3 && pw.length >= 1;
  ...
  const submit = async (event) => {
    ...
    try {
      const payload = isSignup
        ? { mode: 'signup', email, password: pw, name, handle }
        : { mode: 'signin', email, password: pw };
      const handler = window.__handleAuth || (isSignup ? onSignedUp : onSignedIn);
    ...
    <div className="auth">
      ...
      <aside className="auth-brand">
        ...
        <div className="auth-brand-inner">
          ...
          <div className="auth-pitch">
            ...
            <p>
              Curated alpha, validated bounties, talent and live classes — a community of
              opportunities for all builders.
            </p>
          </div>
        </div>
      </aside>
      ...
      <main className="auth-form-wrap">
        <div className="auth-form">
          ...
          <p className="auth-subh">
            {isSignup
              ? 'Free forever — no wallet or card required.'
          ...
          <div className="auth-oauth">
            <button
              ...
              onClick={() => supabaseService.signInWithOAuth('google')}
            >
              <span className="aob-ic" style={{ background: '#fff' }}>
                <svg width="15" height="15" viewBox="0 0 24 24">
                  <path
                    d="M12 11v3h5.3c-.2 1.4-1.6 4-5.3 4-3.2 0-5.8-2.6-5.8-5.8S8.8 6.4 12 6.4c1.8 0 3 .8 3.7 1.4l2.5-2.5C16.5 4 14.4 3 12 3 7 3 3 7 3 12s4 9 9 9c5.2 0 8.7-3.7 8.7-8.8 0-.6-.1-1-.2-1.5H12z"
                    fill="#4285F4"
                  />
                </svg>
              </span>
            ...
            <button
              type="button"
              className="auth-oauth-btn"
              onClick={() => supabaseService.signInWithOAuth('twitter')}
            >
            ...
            <button
              type="button"
              className="auth-oauth-btn"
              onClick={() => supabaseService.signInWithOAuth('apple')}
            >
              <span className="aob-ic" style={{ background: '#0b0b0b' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="#fff">
                  <path d="M16.4 12.8c0-2.4 2-3.6 2.1-3.6-1.1-1.6-2.9-1.9-3.5-1.9-1.5-.2-2.9.9-3.6.9-.8 0-1.9-.9-3.2-.9-1.6.1-3.2 1-4 2.6-1.7 2.9-.5 7.3 1.2 9.7.8 1.2 1.7 2.5 3 2.4 1.2-.1 1.7-.8 3.1-.8s1.9.8 3.1.7c1.3 0 2.1-1.2 2.9-2.3.9-1.4 1.3-2.6 1.3-2.7 0 0-2.4-1-2.4-3.1zM14 7c.7-.8 1.1-2 1-3.1-.9 0-2.1.6-2.7 1.4-.6.7-1.2 1.9-1 3 1 0 2-.5 2.7-1.3z" />
                </svg>
              </span>
              Apple
            </button>
            <button
              className="auth-oauth-btn wallet"
              onClick={() => alert('Wallet sign-in coming soon.')}
            >
              <span className="aob-ic" style={{ background: 'var(--brand-yellow)' }}>
          ...
          <form className="auth-fields" onSubmit={submit}>
            ...
            <label className="field">
              ...
              <input
                ...
                placeholder={isSignup ? 'you@studio.dev' : 'you@studio.dev or @ada'}
                value={email}
            ...
            <label className="field">
              <span className="field-label-row">
                ...
                {!isSignup && (
                  <button type="button" className="field-forgot">
                    Forgot?
                  </button>
                )}
              </span>
            ...
            <button type="submit" className="btn primary lg auth-submit" disabled={busy || !valid}>
            ...
            <p className="auth-fine">
              By continuing you agree to our <span className="link">Terms</span> and{' '}
